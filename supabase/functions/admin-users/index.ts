...
const allowed = (v: unknown, max = 255) => (typeof v === "string" ? v.trim().slice(0, max) : "");
...
async function getActor(req: Request) {
  ...
  const { data: roleRow } = await admin.from("user_roles").select("role").eq("user_id", data.user.id).maybeSingle();
  return { admin, user: data.user, role: roleRow?.role || "client" };
}

async function notifyAdmins(admin: any, fromUserId: string, fromEmail: string, description: string) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!actor || !["admin", "sub_admin"].includes(actor.role)) return json({ error: "unauthorized" }, 401);
    const body = await req.json().catch(() => ({}));
    const action = body.action;
    const admin = actor.admin;

    if (action === "createUser" || action === "createSubadmin") {
      if (action === "createSubadmin" && actor.role !== "admin") return json({ error: "admin_only" }, 403);
      ...
      if (!name || !isEmail(email) || phone.length < 7 || password.length < 6) {
        return json({ error: "invalid_input", hint: "Email real, telefone (>=7 dígitos), senha (>=6 caracteres) obrigatórios" }, 400);
      }
      ...
      if (actor.role === "sub_admin") {
        await notifyAdmins(admin, actor.user.id, actor.user.email || "gestor", `cadastrou ${role === "sub_admin" ? "subadmin" : "usuário"}: ${name} (${email})`);
      }
    ...
    if (action === "addMessages") {
      ...
      const { data: prof } = await admin.from("profiles").select("message_limit").eq("user_id", userId).maybeSingle();
      const newLimit = Number(prof?.message_limit || 0) + amount;
      await admin.from("profiles").update({ message_limit: newLimit }).eq("user_id", userId);
      // Re-enable automation if it was paused due to credits
      await admin.from("instances").update({ automation_paused: false }).eq("user_id", userId);
      await admin.from("notifications").insert({
        user_id: userId,
      ...
      if (actor.role === "sub_admin") {
        await notifyAdmins(admin, actor.user.id, actor.user.email || "gestor", `adicionou ${amount} mensagens ao usuário`);
      }
      return json({ ok: true });
    }
    ...
    if (action === "suspendUser") {
      ...
      await admin.from("profiles").update({ is_suspended: suspended, status: suspended ? "suspended" : "active" }).eq("user_id", userId);
      if (actor.role === "sub_admin") {
        await notifyAdmins(admin, actor.user.id, actor.user.email || "gestor", suspended ? "suspendeu um usuário" : "reativou um usuário");
      }
      return json({ ok: true });
    }
    ...
    if (action === "deleteUser") {
      ...
      if (actor.role === "sub_admin") {
        await notifyAdmins(admin, actor.user.id, actor.user.email || "gestor", "removeu um usuário");
      }
      return json({ ok: true });
    }
    ...
    if (action === "updateUser") {
      ...
      if (!userId) return json({ error: "invalid_input" }, 400);
      const patch: any = {};
      if (typeof body.full_name === "string") patch.full_name = allowed(body.full_name, 120);
      if (typeof body.business_name === "string") patch.business_name = allowed(body.business_name, 120);
      ...
      const authPatch: any = {};
      if (typeof body.email === "string" && isEmail(body.email)) authPatch.email = body.email;
      if (typeof body.password === "string" && body.password.length >= 6) authPatch.password = body.password;
      if (Object.keys(authPatch).length) {
        const r = await admin.auth.admin.updateUserById(userId, authPatch);
        if (r.error) return json({ error: r.error.message }, 400);
      }
      if (actor.role === "sub_admin") await notifyAdmins(admin, actor.user.id, actor.user.email || "gestor", "editou um usuário");
      return json({ ok: true });
    }
    ...
    if (action === "sendNotification") {
      ...
      if (actor.role === "admin") {
        ...
        if (targetRole === "all") {
          ...
          const { data } = await admin.from("user_roles").select("user_id").eq("role", targetRole);
          targets = data || [];
        } else if (body.userId) {
